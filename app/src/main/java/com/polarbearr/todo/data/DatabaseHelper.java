...
public class DatabaseHelper {
    ...
    public static Bundle selectAllData(String isCompletedYn){
        ...
        String sql = "select title, content, _id, dateValue, alarmTime, repeatability from " +
                TODO_TABLE +
                " where isCompletedYn = \'" + isCompletedYn + "\'" +
                "order by dateValue";
...
//    }
    ...
    // 행 삽입
    public static void insertData(Bundle bundle){
        ...
        String isCompletedYn = item.getIsCompletedYn();
        int id = item.getId();

        if(database != null) {
            String sql = "insert into " + TODO_TABLE + "(title, content, dateValue, alarmTime, repeatability, isCompletedYn, _id) values(?, ?, ?, ?, ?, ?, ?)";
            Object[] params = {title, content, date, alarmTime, repeatability, isCompletedYn, id};
    ...
    // 데이터 업데이트
    public static void updateData(Bundle bundle){
        TodoItem item = bundle.getParcelable(TODO_ITEM);
        String title = item.getTitle();
        String content = item.getContent();
        String date = item.getDate();
        ...
        if(database != null){
            String sql = "update " + TODO_TABLE + " set title = \'" + title + "\', content = \'" + content + "\', dateValue = \'" + date + "\', alarmTime = \'" + alarmTime + "\', repeatability =\'" + repeatability + "\', isCompletedYn = \'" + isCompletedYn + "\' where _id = " + id;
            database.execSQL(sql);
        }
    }

    public static int selectGreatestId(){
        String sql = "select _id from " + TODO_TABLE + " order by _id desc";
        Cursor cursor;
        int id = 0;
